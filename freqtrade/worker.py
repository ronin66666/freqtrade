...
class Worker:
    ...
    def _init(self, reconfig: bool) -> None:
        ...
        self.freqtrade = FreqtradeBot(self._config)   #初始化FreqtradeBot 机器人

        internals_config = self._config.get('internals', {})
        self._throttle_secs = internals_config.get('process_throttle_secs',    #机器人迭代循环设置进程限制或最短循环持续时间，默认值为5秒
                                                   PROCESS_THROTTLE_SECS)
        self._heartbeat_interval = internals_config.get('heartbeat_interval', 60)   #心跳间隔，默认值为60秒

        self._sd_notify = sdnotify.SystemdNotifier() if \
            self._config.get('internals', {}).get('sd_notify', False) else None    #sd_notify：默认值为False，如果为True，则启用sd_notify
